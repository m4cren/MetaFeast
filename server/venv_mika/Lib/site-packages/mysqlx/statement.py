...
def parse_table_name(default_schema, table_name, sql_mode=""):
    ...
    return (
        default_schema if len(temp) is 1 else temp[0].strip(quote),
        temp[-1].strip(quote),
    )
...
class ModifyStatement(FilterableStatement):
    ...
    def change(self, doc_path, value):
        ...
        """
        self._update_ops.append(
            UpdateSpec(MySQLxCrud.UpdateOperation.ITEM_REPLACE, doc_path, value)
        )
        return self
    ...
    def array_append(self, doc_path, value):
        ...
        """
        self._update_ops.append(
            UpdateSpec(MySQLxCrud.UpdateOperation.ARRAY_APPEND, doc_path, value)
        )
        return self
...
class SelectStatement(FilterableStatement):
    ...
    def get_sql(self):
        ...
        group_by = (
            " GROUP BY {0}".format(self._grouping_str) if self._has_group_by else ""
        )
        having = " HAVING {0}".format(self._having) if self._has_having else ""
        order_by = " ORDER BY {0}".format(self._sort_str) if self._has_sort else ""
        limit = (
...
class DeleteStatement(FilterableStatement):
    ...
    def __init__(self, table, condition=None):
        super(DeleteStatement, self).__init__(
            target=table, condition=condition, doc_based=False
        )
...
class CreateViewStatement(Statement):
    ...
    def execute(self):
        ...
        definer = " DEFINER = {0}".format(self._definer) if self._definer else ""
        columns = " ({0})".format(", ".join(self._columns)) if self._columns else ""
        view_name = quote_multipart_identifier((self._schema.name, self._name))
        check_option = (
            " WITH {0} CHECK OPTION".format(self._check_option)
            if self._check_option
            else ""
        )
        sql = (
            "CREATE{replace} ALGORITHM = {algorithm}{definer} "
...
class AlterViewStatement(CreateViewStatement):
    ...
    def execute(self):
        ...
        sql = (
            "ALTER ALGORITHM = {algorithm}{definer} "
            "SQL SECURITY {security} VIEW {view_name}{columns} "
            "AS {defined_as}{check_option}"
            "".format(
                algorithm=self._algorithm,
...
class CreateTableStatement(Statement):
    ...
    def table_name(self):
        ...
        return quote_multipart_identifier(
            parse_table_name(self.schema.name, self._name)
        )
    ...
    def add_index(self, index_name, *cols):
        ...
        self._indices.append(
            TableIndex(index_name, TableIndex.INDEX, flexible_params(*cols))
        )
        return self
    ...
    def add_unique_index(self, index_name, *cols):
        ...
        Returns:
            mysqlx.CreateTableStatement: CreateTableStatement object.
        """
        self._u_indices.append(
            TableIndex(index_name, TableIndex.UNIQUE_INDEX, flexible_params(*cols))
        )
        return self
...
class ColumnDef(ColumnDefBase):
    ...
    def _data_type(self):
        type_def = ""
        if self._size and (
            ColumnType.is_numeric(self._type)
            or ColumnType.is_char(self._type)
            or ColumnType.is_binary(self._type)
        ):
    ...
    def _col_definition(self):
        ...
        defn = "{0}{1}{2}{3}{4}".format(
            self._data_type(), null, default, auto_inc, comment
        )
        ...
        if self._ref_table and self._ref_fields:
            ref_table = quote_multipart_identifier(
                parse_table_name(self._default_schema, self._ref_table)
            )
