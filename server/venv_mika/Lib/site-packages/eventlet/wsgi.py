        ...
        try:
            ...
            if hasattr(conn, "send") and hasattr(conn, "recv"):
                ...
                raise NotImplementedError(
                    """eventlet.wsgi doesn't support sockets of type {}""".format(
                        type(conn)
                    )
                )
        ...
        try:
            ...
            if last_errno in BROKEN_SOCK:
                self.server.log.debug(
                    "({}) connection reset by peer {!r}".format(
        ...
        try:
            try:
                ...
                if headers_set and not headers_sent and hasattr(result, "__len__"):
                    ...
                    if not bodyless[0] and "Content-Length" not in [
                        h for h, _v in headers_set[1]
                    ]:
...
If unsure, use eventlet.GreenPool."""
    ...
    try:
        ...
        pool.waitall()
        serv.log.info(
            "({}) wsgi exited, is_accepting={}".format(serv.pid, is_accepting)
        )
        try:
