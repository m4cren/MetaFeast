...
_SERVER_MESSAGES = [
    ...
    (MySQLx.ServerMessages.SESS_AUTHENTICATE_OK, MySQLxSession.AuthenticateOk),
    ...
    (MySQLx.ServerMessages.RESULTSET_COLUMN_META_DATA, MySQLxResultset.ColumnMetaData),
...
class Protocol(object):
    ...
    def get_capabilites(self):
        ...
        self._writer.write_message(MySQLx.ClientMessages.CON_CAPABILITIES_GET, msg)
    ...
    def set_capabilities(self, **kwargs):
        ...
        self._writer.write_message(MySQLx.ClientMessages.CON_CAPABILITIES_SET, msg)

        return self.read_ok()

    def send_auth_start(self, method):
        msg = MySQLxSession.AuthenticateStart(mech_name=method)
        self._writer.write_message(MySQLx.ClientMessages.SESS_AUTHENTICATE_START, msg)
    ...
    def send_auth_continue(self, data):
        msg = MySQLxSession.AuthenticateContinue(auth_data=encode_to_bytes(data))
    ...
    def get_binding_scalars(self, statement):
        ...
        for binding in statement._bindings:
            ...
            scalars[pos] = self.arg_object_to_scalar(
                binding["value"], not statement._doc_based
            )
        return scalars
    ...
    def send_find(self, stmt):
        find = MySQLxCrud.Find(
            ...
            collection=MySQLxCrud.Collection(
                name=stmt.target.name, schema=stmt.schema.name
            ),
        )
    ...
    def send_update(self, statement):
        update = MySQLxCrud.Update(
            data_model=(
                MySQLxCrud.DOCUMENT if statement._doc_based else MySQLxCrud.TABLE
            ),
            collection=MySQLxCrud.Collection(
                name=statement.target.name, schema=statement.schema.name
            ),
        )
        ...
        for update_op in statement._update_ops:
            ...
            if update_op.value is not None:
                opexpr.value.CopyFrom(
                    self.arg_object_to_expr(update_op.value, not statement._doc_based)
                )
    ...
    def send_execute_statement(self, namespace, stmt, args):
        stmt = MySQLxSQL.StmtExecute(
            namespace=namespace, stmt=encode_to_bytes(stmt), compact_metadata=False
        )
        for arg in args:
            value = self._create_any(arg)
            stmt.args.extend([value])
        self._writer.write_message(MySQLx.ClientMessages.SQL_STMT_EXECUTE, stmt)
    ...
    def send_insert(self, statement):
        ...
        if hasattr(statement, "_fields"):
            for field in statement._fields:
                insert.projection.extend(
                    [
                        ExprParser(
                            field, not statement._doc_based
        ...
        for value in statement._values:
            ...
            if isinstance(value, list):
                for val in value:
                    obj = self.arg_object_to_expr(val, not statement._doc_based)
    ...
    def _process_frame(self, msg, rs):
        if msg.type == 1:
            ...
            rs._warnings.append(
                Warning(warningMsg.level, warningMsg.code, warningMsg.msg)
            )
        elif msg.type == 2:
    ...
    def arg_object_to_expr(self, value, allow_relational):
        if value is None:
            return MySQLxExpr.Expr(
                type=MySQLxExpr.Expr.LITERAL, literal=build_null_scalar()
            )
        if isinstance(value, bool):
            return MySQLxExpr.Expr(
                type=MySQLxExpr.Expr.LITERAL, literal=build_bool_scalar(value)
            )
        elif isinstance(value, INT_TYPES):
            return MySQLxExpr.Expr(
                type=MySQLxExpr.Expr.LITERAL, literal=build_int_scalar(value)
            )
        elif isinstance(value, (float)):
            return MySQLxExpr.Expr(
                type=MySQLxExpr.Expr.LITERAL, literal=build_double_scalar(value)
            )
            ...
            try:
                ...
                if expression.has_identifier():
                    return MySQLxExpr.Expr(
                        type=MySQLxExpr.Expr.LITERAL, literal=build_string_scalar(value)
                    )
                return expression
            except:
                return MySQLxExpr.Expr(
            ...
            return MySQLxExpr.Expr(
                type=MySQLxExpr.Expr.LITERAL, literal=build_string_scalar(str(value))
            )
