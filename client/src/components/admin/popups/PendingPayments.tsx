...
const PendingPayments = ({ setIsPendingPayment }: PendingPayment) => {
    ...
    return (
        <div className="fixed bg-black/40 backdrop-blur-[4px] w-full h-screen flex justify-center items-center pointer-events-auto">
            <div className="flex flex-col w-[35vw] h-[40rem] items-center">
                ...
                <div
                    className={`${isClose && "pending-payment-close-animation"} pending-payment-animation bg-gradient-to-t from-darkbrown to-lightbrown rounded-2xl w-[85%] h-[32rem] [box-shadow:0_0_5px_rgba(0,0,0,0.6)_inset,0_0_10px_rgba(0,0,0,0.5)] `}
                ></div>
            </div>
        </div>
    );
